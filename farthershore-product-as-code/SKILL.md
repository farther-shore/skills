...
description: Use when defining or changing the STRUCTURE of a FartherShore product — routes, features, meters, limits, capabilities, policies, or surfaces — i.e. anything in product/product.config.ts. Covers the edit→build→push→checks change loop, determinism rules, and the compiler's allowed/blocked changes. This is contract state — change it in code, never via the API.
metadata:
  version: 1.0.0
---

# Product-as-Code: the build/change loop

Product *structure* is **contract** state: it lives in
`product/product.config.ts` and changes only by editing that file and pushing.
The API/CLI/MCP will reject these writes on a live product (`MANAGED_BY_CODE`).
...
```
product/
  product.config.ts     ← the product definition (a decorated class). Source of truth.
  package.json          ← pins @farthershore/product
...
AGENTS.md               ← per-product rules — READ THIS
```

`product/product.config.ts` exports one decorated `@Product` class. Members are
...
1. **Edit** `product/product.config.ts` (or a module it imports).
...
Confirm with the product's `AGENTS.md` — caps and rules can be product-specific.
