...
description: Use when changing pricing, plans, grants, trials, spend caps, or meters on a FartherShore business; running A/B price experiments; or migrating subscribers between plans. Plans are contract state — change them in business/business.config.ts and push, not via the API. These changes hit real subscribers, so they are confirm-gated.
metadata:
  version: 1.1.0
---
...
`MANAGED_BY_CODE` — you change them by editing `business/business.config.ts` and
...
- **Decreasing a limit's capacity or removing a limit** — **blocked** by the
  compiler. To reduce what a plan offers, create a _new_ plan and migrate
  subscribers rather than shrinking the existing one.
- A plan with no revenue source is rejected unless marked `free`.

## Publishing a paid business

Publishing a paid plan requires the business's **Stripe** connection to be
...
- **confirm**: any change to pricing, grants, trials, caps, or plan shape on a
  live business; promoting/rolling back a variant; migrating subscribers.
