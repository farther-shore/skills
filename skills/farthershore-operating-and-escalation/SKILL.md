...
Day-2 operation: keep a live business healthy through the public surface, and —
...
- **Investigate**: usage flat/zero despite known traffic; a frontend build stuck
  for a long time; a backend unhealthy or not seen recently; `business status`
  showing the latest pushed change hasn't taken effect well past the normal
  propagation window.

## The recurring operate loop

- **Routinely**: check `business status`, `usage summary`, `frontend status`, and
...
Then **stop**. Do not retry destructive or speculative actions on a live business
while waiting.
